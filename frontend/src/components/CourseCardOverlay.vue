<template>
	<div class="min-w-80 max-w-sm rounded-md border-2">
		<iframe
		...
		/>
		<div class="p-5">
			<div v-if="course.data.paid_course" class="mb-3 text-2xl font-semibold">
				{{ course.data.price }}
			</div>
			<div v-if="!readOnlyMode">
				...
				<a
					...
					:href="`/login?redirect-to=/lms/billing/course/${course.data.name}#signup`"
				>
					<Button variant="solid" size="md" class="w-full">
						<template #prefix>
				...
				>
					<Button variant="subtle" class="mt-2 w-full" size="md">
						<template #prefix>
			...
			<div class="space-y-4">
				...
				>
					<Star class="size-4 fill-yellow-500 stroke-1.5 text-transparent" />
					<span class="ml-2">
